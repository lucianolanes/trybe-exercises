...
function formatedBookNames() {
  ...
  const bookInfo = books.map((book) => `${book.name} - ${book.genre} - ${book.author.name}`);
  return bookInfo
}
...
function nameAndAge() {
  return releaseAgeAuthor = books.map((book) => (
   {
      age: (book.releaseYear - book.author.birthYear),
      author: book.author.name,
    }
  )).sort((param1, param2) => param1.age - param2.age);
...
function fantasyOrScienceFictionAuthors() {
  const resultFilter = books.filter((book) => book.genre === 'Ficção Científica' || book.genre === 'Fantasia');
  return resultFilter.map((book) => book.author.name).sort();
